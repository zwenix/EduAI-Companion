    ...
    <div className="relative">
      ...
      {isOpen && (
        <>
          ...
          />
          <div
            className={`absolute right-0 mt-2 w-80 sm:w-96 rounded-[24px] shadow-2xl z-50 overflow-hidden border ${isDarkMode ? "bg-slate-900 border-white/10" : "bg-white border-slate-200"}`}
          >
            <div
              className={`flex justify-between items-center p-4 border-b ${isDarkMode ? "border-white/10" : "border-slate-100"}`}
            >
              <h4
            ...
            <div className="max-h-96 overflow-y-auto custom-scrollbar">
              {notifications.length === 0 ? (
                <div
                  className={`p-8 text-center text-sm ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}
                >
                ...
                notifications.map((n) => (
                  <div
                    key={n.id}
                    className={`p-4 border-b last:border-b-0 flex gap-3 ${!n.read ? (isDarkMode ? "bg-brand-cyan/5" : "bg-brand-cyan/5") : ""} ${isDarkMode ? "border-white/5" : "border-slate-100"}`}
                  >
                    <div
                      className={`w-2 h-2 mt-2 rounded-full shrink-0 ${!n.read ? "bg-brand-cyan" : "bg-transparent"}`}
                    />
                    <div className="flex-1">
                      ...
                      {n.reportData && (
                        <div
                          className={`mt-2 p-2 rounded-lg text-[10px] ${isDarkMode ? "bg-black/20 text-slate-400" : "bg-slate-50 text-slate-600"} border ${isDarkMode ? "border-white/5" : "border-slate-100"}`}
                        >
                          <div className="font-bold mb-1">Feedback:</div>
                          <div className="line-clamp-2">
                            {n.reportData.feedback}
                          </div>
                        </div>
                      )}
                    ...
                    <div className="flex flex-col gap-2">
                      {!n.read && (
                        <button
                          ...
                          className="text-slate-400 hover:text-brand-cyan tooltip"
                          title="Mark as Read"
                        >
                          <Check size={14} />
                        </button>
                      ...
                      <button
                        ...
                        className="text-slate-400 hover:text-rose-500 tooltip"
                        title="Delete"
                      >
                        <Trash2 size={14} />
                      </button>
