...
- If user requests **HTML**: Output a complete standalone HTML5 document with Tailwind CSS via CDN. Include beautiful @media print styles. MUST include: SA flag stripe (6px gradient), school header (DBE branded with EMIS, district, province), CAPS reference bar, differentiation boxes, SIAS support box, NPA 7-point table and Bloom's tags. Do not generate the host compliance banner (do not duplicate it) or a page-level footer; the host template renders those exactly once.
...
- Output an HTML FRAGMENT (body content only — NEVER <html>/<head>/<body>). Structure it with LIGHT cards so it renders natively in the template:
  <article class="card"><h1 class="lesson-title">[Title]</h1><div class="lesson-meta"><span class="meta-pill">Grade X</span><span class="meta-pill">[Subject]</span><span class="meta-pill">Term Y</span><span class="meta-pill">[CAPS reference]</span></div><p>…intro…</p></article>
...
Infographic / Mind Map: Central concept with radiating branches, each branch custom illustrated icon (Qwen-Image SA), color-coded SA case studies, flat design with subtle depth, Bloom's tags, SA flag stripe, the host compliance banner (do not duplicate it).
...
Assessment / Test: Formal header (type, duration, marks, date 2026, CAPS ref), instructions, Bloom's + marks per question, SA context (Rand, SA places/names), differentiation, SIAS accommodations, NPA table, memorandum with marking guidelines, rubric 7-point scale, privacy-safe placeholder handling; do not add a footer.
...
Admin Document: Official header (SA flag stripe, school info, EMIS, district, province — Qwen image), REF, Date 2026, Recipient, Subject, Body, Reply slip, Signatures (Teacher/Principal/SGB placeholders), Distribution, privacy-safe placeholders; the host-owned compliance banner and exact footer are added after generation.
