...
export declare function targetData<T extends TargetId | RemoteTargetId>(targetId: T, queryPurpose: TargetPurpose, path: string): TargetData<T>;
export declare function noChangeEvent(targetId: RemoteTargetId, snapshotVersion: number, resumeToken?: ByteString): RemoteEvent<TargetId>;
export declare function existenceFilterEvent(targetId: RemoteTargetId, syncedKeys: DocumentKeySet, remoteCount: number, snapshotVersion: number, bloomFilter?: api.BloomFilter): RemoteEvent<TargetId>;
export declare function docAddedRemoteEvent(docOrDocs: MutableDocument | MutableDocument[], updatedInTargets?: RemoteTargetId[] | TargetId[], removedFromTargets?: RemoteTargetId[] | TargetId[], activeTargets?: RemoteTargetId[] | TargetId[]): RemoteEvent;
export declare function docUpdateRemoteEvent(doc: MutableDocument, updatedInTargets?: RemoteTargetId[], removedFromTargets?: RemoteTargetId[], limboTargets?: RemoteTargetId[]): RemoteEvent;
export declare function castRemoteEvent<T extends TargetId | RemoteTargetId>(src: unknown): RemoteEvent<T>;
export declare function castTargetIds<T extends TargetId | RemoteTargetId>(src: unknown): T[];
