...
function isIMapValue(obj) {
    ...
    if ('fields' in obj && (obj.fields === null || common0dac00d5_node.isPlainObject(obj.fields))) {
        return true;
    }
    return false;
}
...
function valueToDefaultExpr$1(value) {
    ...
    else if (common0dac00d5_node.isPlainObject(value)) {
        result = _map(value);
    }
...
function vectorToExpr$1(value) {
    ...
    else if (value instanceof common0dac00d5_node.VectorValue) {
        return constant(value);
    }
    else if (Array.isArray(value)) {
        return constant(common0dac00d5_node.vector(value));
    }
    else {
...
function fieldOrExpression$1(value) {
    if (common0dac00d5_node.isString(value)) {
...
class Expression {
    ...
    asBoolean() {
        if (this instanceof BooleanExpression) {
            ...
            throw new common0dac00d5_node.FirestoreError('invalid-argument', `Conversion of type ${typeof this} to BooleanExpression not supported.`);
        }
    }
    subtract(subtrahend) {
...
function _field(nameOrPath, methodName) {
    if (typeof nameOrPath === 'string') {
        if (common0dac00d5_node.DOCUMENT_KEY_NAME === nameOrPath) {
            return new Field(common0dac00d5_node.documentId()._internalPath, methodName);
        }
        return new Field(common0dac00d5_node.fieldPathFromArgument('field', nameOrPath), methodName);
    }
    else {
...
class Constant extends Expression {
     ...
     */
    _toProto(_) {
        common0dac00d5_node.hardAssert(this._protoValue !== undefined, 0x00ed);
    ...
    _readUserData(context) {
        ...
        if (isFirestoreValue(this._protoValue)) {
            ...
            this._protoValue = common0dac00d5_node.parseData(this.value, context);
        }
    }
}
...
class MapValue extends Expression {
    ...
    _toProto(serializer) {
        return common0dac00d5_node.toMapValue(serializer, this.plainObject);
    }
}
/**
 *
...
class FunctionExpression extends Expression {
     ...
     */
    get _optionsUtil() {
        return new common0dac00d5_node.OptionsUtil({});
    }
    /**
     * @private
     * @internal
...
function timestampTruncate(fieldNameOrExpression, granularity, timezone) {
    const internalGranularity = common0dac00d5_node.isString(granularity)
...
class PipelineValueExpression extends Expression {
    ...
    _toProto(jsonProtoSerializer) {
        return common0dac00d5_node.toPipelineValue(this.pipeline._toProto(jsonProtoSerializer));
    }
    /**
     * @internal
     */
...
class Ordering {
    ...
    _toProto(serializer) {
        return {
            mapValue: {
                fields: {
                    direction: common0dac00d5_node.toStringValue(this.direction),
...
function isSelectable(val) {
    const candidate = val;
    return (candidate.selectable && common0dac00d5_node.isString(candidate.alias) && isExpr(candidate.expr));
}
...
function isAliasedAggregate(val) {
    const candidate = val;
    return (common0dac00d5_node.isString(candidate.alias) &&
...
function toPipelineBooleanExpr(f) {
    if (f instanceof common0dac00d5_node.FieldFilter) {
        ...
        switch (f.op) {
            ...
            default:
                common0dac00d5_node.fail(0x9047);
        }
    }
    else if (f instanceof common0dac00d5_node.CompositeFilter) {
        switch (f.op) {
            ...
            default:
                common0dac00d5_node.fail(0x89ea);
        }
    }
...
function toPipeline(query, db) {
    let pipeline;
    if (common0dac00d5_node.isCollectionGroupQuery(query)) {
        pipeline = db.pipeline().collectionGroup(query.collectionGroup);
    }
    else if (common0dac00d5_node.isDocumentQuery$1(query)) {
        pipeline = db.pipeline().documents([common0dac00d5_node.doc(db, query.path.canonicalString())]);
    }
    else {
    ...
    // orders
    const orders = common0dac00d5_node.queryNormalizedOrderBy(query);
...
function selectablesToObject(selectables) {
    ...
    for (const selectable of selectables) {
        ...
        if (typeof selectable === 'string') {
            ...
            common0dac00d5_node.fail(0x5319, { selectable });
        }
        if (result[alias] !== undefined) {
            throw new common0dac00d5_node.FirestoreError('invalid-argument', `Duplicate alias or field '${alias}'`);
        }
...
function aliasedAggregateToMap(aliasedAggregatees) {
    return aliasedAggregatees.reduce((map, selectable) => {
        if (map.get(selectable.alias) !== undefined) {
            throw new common0dac00d5_node.FirestoreError('invalid-argument', `Duplicate alias or field '${selectable.alias}'`);
        }
...
function vectorToExpr(value) {
    if (value instanceof Expression) {
        ...
        const result = constant(common0dac00d5_node.vector(value));
        return result;
    }
    else {
        throw new Error('Unsupported value: ' + typeof value);
...
class AddFields extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...
            args: [common0dac00d5_node.toMapValue(serializer, this.fields)]
        };
    }
    _readUserData(context) {
...
class Define extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common0dac00d5_node.toMapValue(serializer, this.aliasedExpressions)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Aggregate extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [
                common0dac00d5_node.toMapValue(serializer, this.accumulators),
                common0dac00d5_node.toMapValue(serializer, this.groups)
            ]
        };
    }
    _readUserData(context) {
...
class Distinct extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common0dac00d5_node.toMapValue(serializer, this.groups)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
    ...
    _toProto(serializer) {
        return {
            ...
            args: [
                ...
                common0dac00d5_node.toStringValue(this.distanceMeasure)
            ]
        };
    }
    _readUserData(context) {
...
class Limit extends Stage {
    ...
    constructor(limit, options) {
        common0dac00d5_node.hardAssert(!isNaN(limit) && limit !== Infinity && limit !== -Infinity, 0x882c);
        super(options);
        this.limit = limit;
    }
    /**
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common0dac00d5_node.toNumber(serializer, this.limit)]
        };
    }
}
class Offset extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common0dac00d5_node.toNumber(serializer, this.offset)]
        };
    }
}
class Select extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common0dac00d5_node.toMapValue(serializer, this.selections)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Sample extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common0dac00d5_node.toNumber(serializer, this.rate), common0dac00d5_node.toStringValue(this.mode)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Union extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common0dac00d5_node.toPipelineValue(this.other._toProto(serializer))]
        };
    }
    _readUserData(context) {
...
class Replace extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [this.map._toProto(serializer), common0dac00d5_node.toStringValue(Replace.MODE)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
function readUserDataHelper(expressionMap, context) {
    if (common0dac00d5_node.isUserData(expressionMap)) {
...
class Pipeline$1 {
    ...
    removeFields(fieldValueOrOptions, ...additionalFields) {
        // Process argument union(s) from method overloads
        const options = isField(fieldValueOrOptions) || common0dac00d5_node.isString(fieldValueOrOptions)
            ? {}
            : fieldValueOrOptions;
        const fields = isField(fieldValueOrOptions) || common0dac00d5_node.isString(fieldValueOrOptions)
            ? [fieldValueOrOptions, ...additionalFields]
            : fieldValueOrOptions.fields;
        // Convert user land convenience types to internal types
        const convertedFields = fields.map(f => common0dac00d5_node.isString(f) ? field(f) : f);
        // Create stage object
    ...
    select(selectionOrOptions, ...additionalSelections) {
        // Process argument union(s) from method overloads
        const options = isSelectable(selectionOrOptions) || common0dac00d5_node.isString(selectionOrOptions)
            ? {}
            : selectionOrOptions;
        const selections = isSelectable(selectionOrOptions) || common0dac00d5_node.isString(selectionOrOptions)
    ...
    offset(offsetOrOptions) {
        ...
        let options;
        let offset;
        if (common0dac00d5_node.isNumber$1(offsetOrOptions)) {
            options = {};
            offset = offsetOrOptions;
        }
        else {
    ...
    limit(limitOrOptions) {
        // Process argument union(s) from method overloads
        const options = common0dac00d5_node.isNumber$1(limitOrOptions) ? {} : limitOrOptions;
        const limit = common0dac00d5_node.isNumber$1(limitOrOptions)
            ? limitOrOptions
    ...
    distinct(groupOrOptions, ...additionalGroups) {
        // Process argument union(s) from method overloads
        const options = common0dac00d5_node.isString(groupOrOptions) || isSelectable(groupOrOptions)
            ? {}
            : groupOrOptions;
        const groups = common0dac00d5_node.isString(groupOrOptions) || isSelectable(groupOrOptions)
    ...
    replaceWith(valueOrOptions) {
        // Process argument union(s) from method overloads
        const options = common0dac00d5_node.isString(valueOrOptions) || isExpr(valueOrOptions) ? {} : valueOrOptions;
        const fieldNameOrExpr = common0dac00d5_node.isString(valueOrOptions) || isExpr(valueOrOptions)
            ? valueOrOptions
            : valueOrOptions.map;
        // Convert user land convenience types to internal types
    ...
    sample(documentsOrOptions) {
        // Process argument union(s) from method overloads
        const options = common0dac00d5_node.isNumber$1(documentsOrOptions) ? {} : documentsOrOptions;
        let rate;
        let mode;
        if (common0dac00d5_node.isNumber$1(documentsOrOptions)) {
            rate = documentsOrOptions;
            mode = 'documents';
        }
        else if (common0dac00d5_node.isNumber$1(documentsOrOptions.documents)) {
    ...
    unnest(selectableOrOptions, indexField) {
        ...
        if (common0dac00d5_node.isString(indexFieldName)) {
...
class PipelineSource {
    ...
    collection(collectionOrOptions) {
        // Process argument union(s) from method overloads
        const options = common0dac00d5_node.isString(collectionOrOptions) ||
            common0dac00d5_node.isCollectionReference(collectionOrOptions)
            ? {}
            : collectionOrOptions;
        const collectionRefOrString = common0dac00d5_node.isString(collectionOrOptions) ||
            common0dac00d5_node.isCollectionReference(collectionOrOptions)
            ? collectionOrOptions
            : collectionOrOptions.collection;
        // Validate that a user provided reference is for the same Firestore DB
        if (common0dac00d5_node.isCollectionReference(collectionRefOrString)) {
            this._validateReference(collectionRefOrString);
        }
        // Convert user land convenience types to internal types
        const normalizedCollection = common0dac00d5_node.isString(collectionRefOrString)
    ...
    collectionGroup(collectionIdOrOptions) {
        ...
        let collectionId;
        let options;
        if (common0dac00d5_node.isString(collectionIdOrOptions)) {
    ...
    documents(docsOrOptions) {
        ...
        docs
            .filter(v => v instanceof common0dac00d5_node.DocumentReference)
            .forEach(dr => this._validateReference(dr));
        // Convert user land convenience types to internal types
        const normalizedDocs = docs.map(doc => common0dac00d5_node.isString(doc) ? doc : doc.path);
        // Create stage object
    ...
    _validateReference(reference) {
        ...
        if (!refDbId.isEqual(this.databaseId)) {
            throw new common0dac00d5_node.FirestoreError(common0dac00d5_node.Code.INVALID_ARGUMENT, `Invalid ${reference instanceof common0dac00d5_node.CollectionReference
                ? 'CollectionReference'
                : 'DocumentReference'}. ` +
...
function subcollection(pathOrOptions) {
    ...
    let path;
    let options;
    if (common0dac00d5_node.isString(pathOrOptions)) {
        path = pathOrOptions;
        options = {};
    }
    else {
...
class PipelineResult {
    ...
    get(fieldPath) {
        ...
        const value = this._fields.field(common0dac00d5_node.fieldPathFromArgument('DocumentSnapshot.get', fieldPath));
        if (value !== null) {
...
function pipelineResultEqual(left, right) {
    ...
    return (common0dac00d5_node.isOptionalEqual(left._ref, right._ref, common0dac00d5_node.refEqual) &&
        common0dac00d5_node.isOptionalEqual(left._fields, right._fields, (l, r) => l.isEqual(r)));
}
...
function execute(pipelineOrOptions) {
    ...
    if (!pipeline._db) {
        return Promise.reject(new common0dac00d5_node.FirestoreError(common0dac00d5_node.Code.FAILED_PRECONDITION, 'This pipeline was created without a database (e.g., as a subcollection pipeline) and cannot be executed directly. It can only be used as part of another pipeline.'));
    }
    const firestore = common0dac00d5_node.cast(pipeline._db, common0dac00d5_node.Firestore);
    const client = common0dac00d5_node.ensureFirestoreConfigured(firestore);
    const userDataReader = common0dac00d5_node.newUserDataReader(firestore);
    ...
    const structuredPipelineOptions = new common0dac00d5_node.StructuredPipelineOptions(rest, rawOptions);
    structuredPipelineOptions._readUserData(context);
    const structuredPipeline = new common0dac00d5_node.StructuredPipeline(pipeline, structuredPipelineOptions);
    return common0dac00d5_node.firestoreClientExecutePipeline(client, structuredPipeline).then(result => {
        ...
        const docs = result
            ...
            ? new common0dac00d5_node.DocumentReference(firestore, null, element.key)
...
common0dac00d5_node.Firestore.prototype.pipeline = function () {
    return new PipelineSource(this._databaseId, (stages) => {
        return new Pipeline(this, stages);
    });
};

exports._internalPipelineToExecutePipelineRequestProto = common0dac00d5_node._internalPipelineToExecutePipelineRequestProto;
