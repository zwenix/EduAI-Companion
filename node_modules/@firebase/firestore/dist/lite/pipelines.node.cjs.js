...
class OptionsUtil {
    ...
    _getKnownOptions(options, context) {
        const knownOptions = commonA020a6a0_node.ObjectValue.empty();
        ...
        for (const knownOptionKey in this.optionDefinitions) {
            if (this.optionDefinitions.hasOwnProperty(knownOptionKey)) {
                ...
                if (knownOptionKey in options) {
                    ...
                    if (optionDefinition.nestedOptions && commonA020a6a0_node.isPlainObject(optionValue)) {
                        ...
                        protoValue = commonA020a6a0_node.parseData(optionValue, context) ?? undefined;
                    }
                    if (protoValue) {
                        knownOptions.set(commonA020a6a0_node.FieldPath$1.fromServerFormat(optionDefinition.serverName), protoValue);
                    }
                }
            }
        }
    ...
    getOptionsProto(context, knownOptions, optionsOverride) {
        ...
        if (optionsOverride) {
            const optionsMap = new Map(commonA020a6a0_node.mapToArray(optionsOverride, (value, key) => [
                commonA020a6a0_node.FieldPath$1.fromServerFormat(key),
                value !== undefined ? commonA020a6a0_node.parseData(value, context) : null
            ]));
...
function isIMapValue(obj) {
    ...
    if ('fields' in obj && (obj.fields === null || commonA020a6a0_node.isPlainObject(obj.fields))) {
        return true;
    }
    return false;
}
...
function valueToDefaultExpr$1(value) {
    ...
    else if (commonA020a6a0_node.isPlainObject(value)) {
        result = _map(value);
    }
...
function vectorToExpr$1(value) {
    ...
    else if (value instanceof commonA020a6a0_node.VectorValue) {
        return constant(value);
    }
    else if (Array.isArray(value)) {
        return constant(commonA020a6a0_node.vector(value));
    }
    else {
...
function fieldOrExpression$1(value) {
    if (commonA020a6a0_node.isString(value)) {
...
class Expression {
    ...
    asBoolean() {
        if (this instanceof BooleanExpression) {
            ...
            throw new commonA020a6a0_node.FirestoreError('invalid-argument', `Conversion of type ${typeof this} to BooleanExpression not supported.`);
        }
    }
    subtract(subtrahend) {
...
function _field(nameOrPath, methodName) {
    if (typeof nameOrPath === 'string') {
        if (commonA020a6a0_node.DOCUMENT_KEY_NAME === nameOrPath) {
            return new Field(commonA020a6a0_node.documentId()._internalPath, methodName);
        }
        return new Field(commonA020a6a0_node.fieldPathFromArgument('field', nameOrPath), methodName);
    }
    else {
...
class Constant extends Expression {
     ...
     */
    _toProto(_) {
        commonA020a6a0_node.hardAssert(this._protoValue !== undefined, 0x00ed);
    ...
    _readUserData(context) {
        ...
        if (isFirestoreValue(this._protoValue)) {
            ...
            this._protoValue = commonA020a6a0_node.parseData(this.value, context);
        }
    }
}
...
class MapValue extends Expression {
    ...
    _toProto(serializer) {
        return commonA020a6a0_node.toMapValue(serializer, this.plainObject);
    }
}
/**
 *
...
function timestampTruncate(fieldNameOrExpression, granularity, timezone) {
    const internalGranularity = commonA020a6a0_node.isString(granularity)
...
class PipelineValueExpression extends Expression {
    ...
    _toProto(jsonProtoSerializer) {
        return commonA020a6a0_node.toPipelineValue(this.pipeline._toProto(jsonProtoSerializer));
    }
    /**
     * @internal
     */
...
class Ordering {
    ...
    _toProto(serializer) {
        return {
            mapValue: {
                fields: {
                    direction: commonA020a6a0_node.toStringValue(this.direction),
...
function isSelectable(val) {
    const candidate = val;
    return (candidate.selectable && commonA020a6a0_node.isString(candidate.alias) && isExpr(candidate.expr));
}
...
function isAliasedAggregate(val) {
    const candidate = val;
    return (commonA020a6a0_node.isString(candidate.alias) &&
...
function toPipelineBooleanExpr(f) {
    if (f instanceof commonA020a6a0_node.FieldFilter) {
        ...
        switch (f.op) {
            ...
            default:
                commonA020a6a0_node.fail(0x9047);
        }
    }
    else if (f instanceof commonA020a6a0_node.CompositeFilter) {
        switch (f.op) {
            ...
            default:
                commonA020a6a0_node.fail(0x89ea);
        }
    }
...
function toPipeline(query, db) {
    let pipeline;
    if (commonA020a6a0_node.isCollectionGroupQuery(query)) {
        pipeline = db.pipeline().collectionGroup(query.collectionGroup);
    }
    else if (commonA020a6a0_node.isDocumentQuery(query)) {
        pipeline = db.pipeline().documents([commonA020a6a0_node.doc(db, query.path.canonicalString())]);
    }
    else {
    ...
    // orders
    const orders = commonA020a6a0_node.queryNormalizedOrderBy(query);
...
function selectablesToObject(selectables) {
    ...
    for (const selectable of selectables) {
        ...
        if (typeof selectable === 'string') {
            ...
            commonA020a6a0_node.fail(0x5319, { selectable });
        }
        if (result[alias] !== undefined) {
            throw new commonA020a6a0_node.FirestoreError('invalid-argument', `Duplicate alias or field '${alias}'`);
        }
...
function aliasedAggregateToMap(aliasedAggregatees) {
    return aliasedAggregatees.reduce((map, selectable) => {
        if (map.get(selectable.alias) !== undefined) {
            throw new commonA020a6a0_node.FirestoreError('invalid-argument', `Duplicate alias or field '${selectable.alias}'`);
        }
...
function vectorToExpr(value) {
    if (value instanceof Expression) {
        ...
        const result = constant(commonA020a6a0_node.vector(value));
        return result;
    }
    else {
        throw new Error('Unsupported value: ' + typeof value);
...
class AddFields extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...
            args: [commonA020a6a0_node.toMapValue(serializer, this.fields)]
        };
    }
    _readUserData(context) {
...
class Define extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonA020a6a0_node.toMapValue(serializer, this.aliasedExpressions)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Aggregate extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [
                commonA020a6a0_node.toMapValue(serializer, this.accumulators),
                commonA020a6a0_node.toMapValue(serializer, this.groups)
            ]
        };
    }
    _readUserData(context) {
...
class Distinct extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonA020a6a0_node.toMapValue(serializer, this.groups)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
    ...
    _toProto(serializer) {
        return {
            ...
            args: [
                ...
                commonA020a6a0_node.toStringValue(this.distanceMeasure)
            ]
        };
    }
    _readUserData(context) {
...
class Limit extends Stage {
    ...
    constructor(limit, options) {
        commonA020a6a0_node.hardAssert(!isNaN(limit) && limit !== Infinity && limit !== -Infinity, 0x882c);
        super(options);
        this.limit = limit;
    }
    /**
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonA020a6a0_node.toNumber(serializer, this.limit)]
        };
    }
}
class Offset extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonA020a6a0_node.toNumber(serializer, this.offset)]
        };
    }
}
class Select extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonA020a6a0_node.toMapValue(serializer, this.selections)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Sample extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonA020a6a0_node.toNumber(serializer, this.rate), commonA020a6a0_node.toStringValue(this.mode)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Union extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonA020a6a0_node.toPipelineValue(this.other._toProto(serializer))]
        };
    }
    _readUserData(context) {
...
class Replace extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [this.map._toProto(serializer), commonA020a6a0_node.toStringValue(Replace.MODE)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
function readUserDataHelper(expressionMap, context) {
    if (commonA020a6a0_node.isUserData(expressionMap)) {
...
class Pipeline {
    ...
    removeFields(fieldValueOrOptions, ...additionalFields) {
        // Process argument union(s) from method overloads
        const options = isField(fieldValueOrOptions) || commonA020a6a0_node.isString(fieldValueOrOptions)
            ? {}
            : fieldValueOrOptions;
        const fields = isField(fieldValueOrOptions) || commonA020a6a0_node.isString(fieldValueOrOptions)
            ? [fieldValueOrOptions, ...additionalFields]
            : fieldValueOrOptions.fields;
        // Convert user land convenience types to internal types
        const convertedFields = fields.map(f => commonA020a6a0_node.isString(f) ? field(f) : f);
        // Create stage object
    ...
    select(selectionOrOptions, ...additionalSelections) {
        // Process argument union(s) from method overloads
        const options = isSelectable(selectionOrOptions) || commonA020a6a0_node.isString(selectionOrOptions)
            ? {}
            : selectionOrOptions;
        const selections = isSelectable(selectionOrOptions) || commonA020a6a0_node.isString(selectionOrOptions)
    ...
    offset(offsetOrOptions) {
        ...
        let options;
        let offset;
        if (commonA020a6a0_node.isNumber(offsetOrOptions)) {
            options = {};
            offset = offsetOrOptions;
        }
        else {
    ...
    limit(limitOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonA020a6a0_node.isNumber(limitOrOptions) ? {} : limitOrOptions;
        const limit = commonA020a6a0_node.isNumber(limitOrOptions)
            ? limitOrOptions
    ...
    distinct(groupOrOptions, ...additionalGroups) {
        // Process argument union(s) from method overloads
        const options = commonA020a6a0_node.isString(groupOrOptions) || isSelectable(groupOrOptions)
            ? {}
            : groupOrOptions;
        const groups = commonA020a6a0_node.isString(groupOrOptions) || isSelectable(groupOrOptions)
    ...
    replaceWith(valueOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonA020a6a0_node.isString(valueOrOptions) || isExpr(valueOrOptions) ? {} : valueOrOptions;
        const fieldNameOrExpr = commonA020a6a0_node.isString(valueOrOptions) || isExpr(valueOrOptions)
            ? valueOrOptions
            : valueOrOptions.map;
        // Convert user land convenience types to internal types
    ...
    sample(documentsOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonA020a6a0_node.isNumber(documentsOrOptions) ? {} : documentsOrOptions;
        let rate;
        let mode;
        if (commonA020a6a0_node.isNumber(documentsOrOptions)) {
            rate = documentsOrOptions;
            mode = 'documents';
        }
        else if (commonA020a6a0_node.isNumber(documentsOrOptions.documents)) {
    ...
    unnest(selectableOrOptions, indexField) {
        ...
        if (commonA020a6a0_node.isString(indexFieldName)) {
...
class PipelineSource {
    ...
    collection(collectionOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonA020a6a0_node.isString(collectionOrOptions) ||
            commonA020a6a0_node.isCollectionReference(collectionOrOptions)
            ? {}
            : collectionOrOptions;
        const collectionRefOrString = commonA020a6a0_node.isString(collectionOrOptions) ||
            commonA020a6a0_node.isCollectionReference(collectionOrOptions)
            ? collectionOrOptions
            : collectionOrOptions.collection;
        // Validate that a user provided reference is for the same Firestore DB
        if (commonA020a6a0_node.isCollectionReference(collectionRefOrString)) {
            this._validateReference(collectionRefOrString);
        }
        // Convert user land convenience types to internal types
        const normalizedCollection = commonA020a6a0_node.isString(collectionRefOrString)
    ...
    collectionGroup(collectionIdOrOptions) {
        ...
        let collectionId;
        let options;
        if (commonA020a6a0_node.isString(collectionIdOrOptions)) {
    ...
    documents(docsOrOptions) {
        ...
        docs
            .filter(v => v instanceof commonA020a6a0_node.DocumentReference)
            .forEach(dr => this._validateReference(dr));
        // Convert user land convenience types to internal types
        const normalizedDocs = docs.map(doc => commonA020a6a0_node.isString(doc) ? doc : doc.path);
        // Create stage object
    ...
    _validateReference(reference) {
        ...
        if (!refDbId.isEqual(this.databaseId)) {
            throw new commonA020a6a0_node.FirestoreError(commonA020a6a0_node.Code.INVALID_ARGUMENT, `Invalid ${reference instanceof commonA020a6a0_node.CollectionReference
                ? 'CollectionReference'
                : 'DocumentReference'}. ` +
...
function subcollection(pathOrOptions) {
    ...
    let path;
    let options;
    if (commonA020a6a0_node.isString(pathOrOptions)) {
        path = pathOrOptions;
        options = {};
    }
    else {
...
class PipelineResult {
    ...
    get(fieldPath) {
        ...
        const value = this._fields.field(commonA020a6a0_node.fieldPathFromArgument('DocumentSnapshot.get', fieldPath));
        if (value !== null) {
...
function execute(pipeline) {
    if (!pipeline._db) {
        return Promise.reject(new commonA020a6a0_node.FirestoreError(commonA020a6a0_node.Code.FAILED_PRECONDITION, 'This pipeline was created without a database (e.g., as a subcollection pipeline) and cannot be executed directly. It can only be used as part of another pipeline.'));
    }
    const datastore = commonA020a6a0_node.getDatastore(pipeline._db);
    const firestore = commonA020a6a0_node.cast(pipeline._db, commonA020a6a0_node.Firestore);
    const userDataReader = commonA020a6a0_node.newUserDataReader(firestore);
    ...
    return commonA020a6a0_node.invokeExecutePipeline(datastore, structuredPipeline).then(result => {
        ...
        const docs = result
            ...
            ? new commonA020a6a0_node.DocumentReference(firestore, null, element.key)
 ...
 * ```
 */
commonA020a6a0_node.Firestore.prototype.pipeline = function () {
